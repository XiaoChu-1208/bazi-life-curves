...
## 0. 典型边界 case（本协议的诞生原因）

一类典型边界结构：日主己土 + 月支水 + 干头多火多燥土：

- **默认 Skill 推法**：日主主导 + 印星化杀 + 身弱用印 → R0/R1 命中率 ≈ 30%
- **反向推法**：丙火财星主事 + 日主借力 + 上燥下寒 → R0/R1 命中率 ≈ 90%
...
> v7 旧版只看日支主气一个字段，导致典型边界 case (日主己土 + 时柱己巳) 被判反为"真从"
> （日支非同党 → 旧 has_root=False → 真从），实际诊断应是"假从财格"
> （时干同党 + 时支巳藏丙印有根 → 应判假从）。v7.1 改为综合 4 字段判断。
...
- **正例**：典型边界结构 (日主己土 + 干头多火 + 月支水) → P5 4/4 全触发 → suggested_phase = `floating_dms_to_cong_cai`，跟人肉诊断的"假从财格 / 财官印三气成象"完全一致
...
- ⚠️ P4 真从格触发 → `true_following`（**方向判反了**——人肉诊断为"假从财"）
- ❌ P1 日主虚浮 2/5 不触发（漏掉 floating_dms_to_cong_cai）
- ❌ P2 旺神得令 1/4 不触发

**人肉诊断**（凭经验）：「**假从财格 / 财官印三气成象**」+「**上燥下寒**」
...
|---|---|---|
| **P1 阈值过严** | `support ≤ 2` 把典型边界 case 的 support=4.6 排除；`无印` 是死条件，把"假从有印"全排除 | 改为「比劫不通根（看地支主气）」+「印不通根（看印是否有支撑）」；旺神阈值 12 → 8 |
...
| **缺 P5 三气成象 detect** | 没有处理"≥ 2 个非日主五行成势 + 流通生化"的复杂场景 | 新增 P5，强制要求 4 条件全满足（含连环生化），典型边界 case 触发 4/4 |

### 11.3 v7.1 后的边界 case 输出
...
| 典型边界 case (日主己土 + 干头多火 + 月支水) | 弱 -30 | 至少触发 P3 + P5 | ✓ 3 candidates，方向准确 |
...
**修复**：`scripts/handshake.py` 加 `--phase-id <phase_id>` 参数，用反演后的 strength/yongshen/climate **重生成 R0/R1/R2 6 题**。例如典型边界 case：
