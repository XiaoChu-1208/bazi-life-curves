...
> 一类典型边界 case（详见 [diagnosis_pitfalls.md §13-14](diagnosis_pitfalls.md)）
> 是这个红线的诞生现场——一次 0.83 后验的"假从财格"叙事差点彻底盖过反例事件。
...
```json
{
  "phase": {
    "id": "<primary_phase_id>",
    "must_be_true": [
      {"prediction": "在主格关键岁运区间必有可验证的对应类型大事件", "evidence_required": "具体年份 + 事件类型"},
      ...
    ],
    "must_be_false": [
...
## §5 关于"高后验假从误判"模式的最终承诺

这一类边界 case ("印根足够却被算法判为弃命从财") 是 v9 全部 7 个 PR 的诞生原因。
从此以后，类似它的"高后验单 phase 强行覆盖反例事件"的模式，必须在 6 个独立位点被拦截：
...
任何一个失守，下一次同类误判都会再发生。所以这 6 个位点必须
