...
Step 2.5 对每份八字跑 handshake.py → 健康三问
        ...
        （v9.3 已删除原 Step 2.7 询问输出格式：合盘默认就走 markdown 流式，
         HTML 汇总表仅在用户**主动**说「要 HTML / 给我图 / 出 artifact」时才跑——
         严禁主动询问「要 markdown 还是 HTML」、严禁默认渲染 HTML。
         合盘场景下 HTML 不是刚需；分析阶段优先 streaming markdown。）
...
**v5 流式硬要求**：上述 10 节必须按节流式输出，禁止把整段汇总憋到末尾一次性吐。
**v9.3 默认输出**：合盘默认就走纯 markdown 流式；HTML 汇总表仅当用户**主动**说「要 HTML / 给我图 / 出 artifact」时才跑——绝对不主动询问「要 markdown 还是 HTML」、绝对不主动渲染 HTML。
**v9.3 R-STREAM-1/R-STREAM-2 同样适用**：每个 `append_analysis_node` 调用之间必须有 stop turn；单条 assistant message 的 user-facing markdown 不允许 ≥2 个顶级 `## ` heading（详见 `AGENTS.md` v9.3 红线）。
