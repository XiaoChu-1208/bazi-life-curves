...
2. **合盘**：用户输入 ≥ 2 份八字 + 关系类型（合作 / 婚配 / 友谊 / 家人），脚本算出 4 层结构性评分（五行互补 + 干支互动 + 十神互配 + 大运同步度），LLM 在对话里给出"亮点 + 摩擦点 + 怎么用 / 怎么避"的人话解读。**v9.3 起合盘整体走 v9 adaptive_elicit 多人编排（`he_pan_orchestrator --mode plan-v9 / next-person`）：每份八字独立 finalize phase 后再合盘；旧 R0/R1 健康三问路径已退役（仅 `--ack-batch` / `BAZI_HEPAN_BYPASS_V8_GATE=1` 兜底）。详见 §2.6。**
...
2. **出图 / 合盘前**我会跑 v8 校验回路（一次抛 ~20-25 道结构化点选题）：
   ...
   - 合盘场景（v9.3）：每份八字独立走相同的 v9 adaptive_elicit 校验，由 `he_pan_orchestrator --mode plan-v9 / next-person` 串行驱动；旧 R0/R1 健康三问路径已退役（详见 §2.6）
...
| **D4 · 中医体征** | 6 | hard_evidence × 2.0 | 寒热 / 睡眠 / 脏腑薄弱处 / 体型体格 / 食欲口味 / 情志倾向（扩展旧 R1 三问 · v8 已退役 · 到完整六问） |
...
| HS-R6 | **open_phase anchor UI 自创硬约束** | agent 在"补事件年份"prompt / askquestion_payload / UI label 里加任何协议未规定的 filter（年龄段 "25 岁前" / 事件类型 / 强度阈值 / 地理 / 身份）→ `_v9_guard.scan_fabricated_anchor_constraint` 命中 | exit 12，强制改文案为"请补 ≥ 2 个你能确认的具体公历年 + 事件描述"，不加 filter |
| HS-R6.1 | **open_phase anchor 假反馈** | 用户提交 anchor 后 agent 回"等待重跑校准"但实际没真调 `multi_school_vote.vote()` 重算 | 必须真跑 vote()，并把 top1_posterior / decision 的实际变化逐项告知用户 |
| HS-R6.2 | **open_phase anchor 静默 filter** | 用户输入被判无效但 UI 只显示"已识别 0 个有效年份"不解释原因 | 必须逐条告知"为什么 0"（年份解析失败 / 描述为空 / etc），禁止笼统 |
...
> **v9.3 已对齐单盘**：合盘场景与单盘走**同一套** v9 adaptive_elicit 路径。
> 由 `he_pan_orchestrator.py` 编排多人 phase decision 状态，每份八字独立 finalize（`is_provisional=false` 且 `confidence ≥ 0.60`）后再调 `he_pan.py` 算 4 层评分。
> 旧 R0 反询问 / 旧 R1 三问（v8 已退役）/ 旧双方命中率判定路径全部已退役，仅 `--ack-batch` / `BAZI_HEPAN_BYPASS_V8_GATE=1` 兜底。
> `virtue_motifs.py` 必须每人各跑一次（→ `## 我想和你说 · <name>` 三段闭合 + 共振 motif 取交集）。
> 完整规则见 `references/he_pan_protocol.md`（v2）。

```bash
# Step A · 多人编排：plan-v9 列出每人 phase decision 状态
python scripts/he_pan_orchestrator.py --mode plan-v9 \
  --bazi /tmp/p1_bazi.json /tmp/p2_bazi.json \
  --names 男方 女方

# Step B · 串行 next-person：对当前未 finalize 的人跑 adaptive_elicit
python scripts/he_pan_orchestrator.py --mode next-person \
  --bazi /tmp/p1_bazi.json /tmp/p2_bazi.json \
  --names 男方 女方 \
  --out-dir /tmp/hepan_state/

# Step C · 每人 finalize 后再各跑一次 virtue_motifs.py
python scripts/virtue_motifs.py --bazi /tmp/p1_bazi.json --curves /tmp/p1_curves.json --out /tmp/p1_virtue.json
python scripts/virtue_motifs.py --bazi /tmp/p2_bazi.json --curves /tmp/p2_curves.json --out /tmp/p2_virtue.json

# Step D · 4 层合盘评分（he_pan.py 入口守门：每人 phase 必须 finalized + virtue_motifs 必须存在）
...
**LLM 解读规则（强制 · v9.3 流式输出 · 13 节）**：

**流式分节顺序**（每写完一节立刻发出，**禁止憋整段** · `R-STREAM-1/2` 物理硬 lint）：

```
[Node 1]  ## 概览                       双方八字 + 关系类型 + confidence（基于双方 phase.confidence）
[Node 2]  ## 总分定调                   人话定调（不甩 grade）—— 1 段
[Node 3]  ## 第 1 层 · 五行互补
[Node 4]  ## 第 2 层 · 干支互动         标出关键合 / 冲
[Node 5]  ## 第 3 层 · 十神互配         按 rel_type 重点
[Node 6]  ## 第 4 层 · 大运同步         focus_years 哪些年同向 / 反向
[Node 7]  ## 关键加分项                 top_pluses（怎么用）
[Node 8]  ## 关键减分项                 top_minuses（怎么避）
[Node 9]  ## 关系类型特定 tips          婚配 / 合作 / 友谊 / 家人 各自要点
[Node 10] ## 总结                       1 段大白话 + "如果决定做 X，怎样能更顺"
─── 合盘 closing 三段（每人各一组，virtue_motifs 驱动）─────────────
[Node 11] ## 我想和你说 · <name1>       第 1 人 virtue_narrative 三段（同序拼接）
[Node 12] ## 我想和你说 · <name2>       第 2 人同上
[Node 13] ## 共振 motif                 ∩ shared_motif_ids → 1 段共振解读（无交集时显式说"两位的人性主题不重叠"）
```
...
- ❌ 直接给"配 / 不配"二元结论
- ❌ 把 grade 直接报给用户
- ❌ 跳过任一方 phase finalize（`is_provisional=true` 或 `confidence < 0.60`）直接合盘 → he_pan.py 入口 exit 7
- ❌ 缺任一方 virtue_motifs.json 直接合盘 → he_pan.py 入口 exit 7（除非 `BAZI_HEPAN_SKIP_VIRTUE=1` 兜底）
- ❌ 不援引脚本 notes 凭"感觉"打分
- ❌ 给"建议分手 / 建议结婚 / 建议合伙"代替用户做决定的话——只给"如果决定做 X，怎样能更顺"
- ❌ **把所有节点憋在末尾一次性吐**（R-STREAM 物理硬要求 —— 每节立即发出）
...
### v9.3.1 · open_phase anchor 收集铁律（防 agent 编造 UI 约束）

落 open_phase 时 agent 向用户收 anchor，**唯一能 filter 的是**：

1. 年份是否能解析为合法公历年（YYYY 格式）
2. 事件描述是否非空

**绝对不得加**任何下列 filter（HS-R6 红线，`_v9_guard.scan_fabricated_anchor_constraint` 机械拦截）：

- ❌ 年龄段约束（"25 岁前 / 成年前 / 童年期 / 本命大运前"）
- ❌ 事件类型 filter（"只收事业 / 只收学业 / 只收感情"）
- ❌ 强度阈值（"只收大事件 / 改变人生轨迹的"）
- ❌ 地理 / 身份 / 关系状态 filter（"只收国内 / 只收已婚后"）

**必须**：

- 实时显式校验反馈（HS-R6.2 · 哪条无效说原因，禁止只显示"已识别 0 个有效年份"）
- 提交按钮真触发 `multi_school_vote.vote(bazi, fallback_phase_candidates=anchors)` 重算（HS-R6.1 · 不许只回"等待重跑校准"安慰话术）
- 把 `top1_posterior` / `decision` 的实际变化逐项告知用户

完整规则 + 历史 incident 复盘见 [`references/open_phase_anchor_protocol.md`](references/open_phase_anchor_protocol.md)。
...
| `confidence_note` | 双方 phase.confidence（v9 adaptive_elicit 后验）→ 合盘 confidence + 必要的 caveat |
