...
<!--
  ...
  MCP server, Model Context Protocol, bazi MCP, 八字 MCP, Claude Desktop MCP,
  Cursor MCP, lunar-python, tyme4py, sxtwl, 寿星天文历, 真太阳时, EOT, equation of time,
  cantian-ai compatible, 节气精度, 命理引擎
-->
...
v9 的全部 7 个 PR 源于这样一类典型的失败模式：

某些八字结构看起来"日主几乎无根"——比如日主土星但月支水旺 + 财星杂多 + 印星藏在地支不透干。旧 v8 的算法链:

1. `compute_dayuan_root_strength` 没有「本气/中气/余气」三档分级 → 把藏在地支主气里的印星贡献漏算
2. 日主被算法判为「无根」→ 触发「弃命从财格」候选
3. 单流派（滴天髓系）独断, 后验 ≥ 0.80 → adopt
4. 输出："你是真从财格, 印比岁运反成阻力, 财官岁运为大利"

而真实情况是：地支主气里的印星本来贡献 1.0 的根, 整个日主的 yin_root 完全够格——这类盘根本不是从格, 而是杀印相生 / 伤官生财 / 调候反向 等多格并存。如果用户在印星岁运 (按算法标记为"忌") 实际经历的是贵人型大事件, 算法就会把这条**反例证据**强行解读成"应验"——叙事一旦立起来, 反例越多反而被算法越当作支持证据。
...
### 6 个 PR 的修复链 · 每个独立位点都防一次同类误判重演

| PR | 修复深层缺陷 | 关键文件 | 学理理由 |
|---|---|---|---|
| **PR-1** 通根度严判 | 粒度缺陷 | `_bazi_core.py::compute_dayuan_root_strength` (本气1.0 / 中气0.5 / 余气0.2) + `score_curves.py::apply_phase_override` 守卫 | 《滴天髓》"通根透干, 全在生扶"——通根有强弱之分, 不是 0/1。地支主气里的印星贡献 1.0 的根, 全盘 yin_root 充足时不应被标为"无根" |
...
| **PR-3** 大运层 fanyin/fuyin | 时间维度遗漏 | `mangpai_events.py::detect_dayun_*` 4 个新 detector + `dayun_only` 首年触发机制 (避 10x 重复) | 盲派"反吟伏吟"不只看流年与命局, 还要看大运与命局、流年与大运。某些盘的大运起伏全在大运层, 旧 detector 只看流年层完全捞不到 |
...
| **PR-6** 多流派加权投票 + open_phase 逃逸阀 | 独断缺陷 + 逃逸缺陷 | `_school_registry.py` (新, 6 大流派) + `multi_school_vote.py` (新) | 子平真诠 (1.0) / 滴天髓 (1.0) / 穷通宝鉴 (0.9) / 盲派 (0.9) / 紫微 (0.3 ratify) / 铁板 (0.3 ratify) 加权投票 → top1<0.55 OR gap<0.10 → `decision="open_phase"`. 多格并存型盘 (杀印相生 + 伤官生财 + 调候反向 等三足鼎立) 在 v9 必落 open_phase, 不允许独断 |
...
3. **请用户补充 ≥ 2 条具体事件年份** (升学 / 工作 / 婚配 / 大病 / 迁徙等), 重新跑投票
...
### 端到端 v9 验证 · 多格并存型边界 case

对一个"印根足够 + 财杀混杂 + 调候反向" 的边界 case 跑全流程:

```bash
$ python3 scripts/multi_school_vote.py --bazi /tmp/case.json
{
  "version": "v9-PR6",
...
v9 不再独断这类盘是"弃命从财格", 而是把多个 phase 都列出, 让用户用更多事件锚点决定方向——这才是"工具该做的事"。

### v9 测试覆盖

每个 PR 都有专属测试模块 + 边界 case 回归断言:

| 测试模块 | 用例数 | 边界 case 关键断言 |
|---|---:|---|
| `tests/test_root_strength.py` | 13 | 地支主气印星 → yin_root ≥ 1.0, label != "无根"/"微根" |
